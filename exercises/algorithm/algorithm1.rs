...
impl<T> LinkedList<T> {
        ...
        T: Ord,
    {
        ...
        unsafe {
            // 尾插一个节点到结果链表（断开其原 next，接到 tail 后面）
            let mut push = |node: NonNull<Node<T>>| {
            ...
            while let (Some(ap), Some(bp)) = (a, b) {
                // 用引用比较，避免移动 T
                let take_a = &(*ap.as_ptr()).val <= &(*bp.as_ptr()).val;

                if take_a {
                    let next = (*ap.as_ptr()).next; // 先记住下一个
                    push(ap); // 再把当前节点接到结果尾
                    a = next; // 前进
                } else {
                    let next = (*bp.as_ptr()).next;
                    push(bp);
                    b = next;
                }
            }
